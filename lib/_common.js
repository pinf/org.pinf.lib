							...
							var Context = function (API, rootPath) {
								...
								self.console = {
									error: function () {
										...
										self.LOGGER.log.call(self.LOGGER, "error", (args.length > 1 ? args : args[0]));
									},
									verbose: function () {
										if (!self.VERBOSE) return;
										var args = Array.prototype.slice.call(arguments);
										self.LOGGER.log.call(self.LOGGER, "info", (args.length > 1 ? args : args[0]));
									},
									debug: function () {
										if (!self.DEBUG) return;
										var args = Array.prototype.slice.call(arguments);
										self.LOGGER.log.call(self.LOGGER, "debug", (args.length > 1 ? args : args[0]));
									}
								}
								self.loadProgramDescriptor = function (callback) {
									var programDescriptorPath = rootPath;
