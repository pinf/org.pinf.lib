...
exports.fromFile = function (proto, path, callback) {
	...
	var loadRootPINFConfigOnce = function (callback) {
		...
		return loadFromFile(PATH.join(process.env.PGS_WORKSPACE_ROOT, "PINF.json"), function (err, pinfConfig) {
			if (err) return callback(err);

			var descriptor = new proto(PATH.join(process.env.PGS_WORKSPACE_ROOT, "PINF.json"), pinfConfig);
...
*/			
	...
	return loadFromFiles(function (err, data) {
		if (err) return callback(err);
		var descriptor = new proto(path, data);
		return descriptor.init(function (err) {
			if (err) return callback(err);
...
var Descriptor = exports.Descriptor = function (proto, path, data) {
	var self = this;
	self._proto = proto;
	self._path = path;
	self._data = data;
...
//console.log("DATA", self._data);
	...
	function resolveVariables () {
		...
		for (var name in envVars) {
			...
			configString = configString.replace(new RegExp(ESCAPE_REGEXP_COMPONENT(envVars[name].matched), "g"), process.env[name]);
		}
			...
			return waitfor(function(callback) {
				...
				if (/^\//.test(locator)) {
					...
					return FS.exists(path, function(exists) {
						...
						return exports.fromFile(self._proto, path, function (err, descriptor) {
							if (err) return callback(err);
		...
		for (var configId in self._data.config) {
			...
			waitfor(configId, uri, function (configId, uri, callback) {
				...
				return FS.exists(uri, function (exists) {
					...
					return exports.fromFile(self._proto, uri, function (err, descriptor) {
						if (err) return callback(err);
